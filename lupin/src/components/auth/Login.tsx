...
function useGoogleLogin(
  ...
  const triggerGoogleLogin = useCallback(() => {
    ...
    if (!scriptLoadedRef.current) {
      ...
      const checkAndClick = (attempts = 0) => {
        ...
        if (scriptLoadedRef.current) {
          const hiddenBtn = googleBtnRef.current?.querySelector(
            'div[role="button"]'
          ) as HTMLElement;
          hiddenBtn?.click();
        } else {
  ...
  return {
    googleBtnRef,
    triggerGoogleLogin,
    preloadGoogleScript: loadGoogleScript,
  };
}
...
const SocialButton = memo(
  ({
    ...
    <button
      ...
      className={`w-11 h-11 rounded-full flex items-center justify-center transition-transform hover:scale-110 shadow-md border overflow-hidden bg-white cursor-pointer ${className}`}
      aria-label={`${alt} 로그인`}
      title={`${alt}로 로그인`}
    >
      <img
...
export default function Login() {
  ...
  const { googleBtnRef, triggerGoogleLogin, preloadGoogleScript } =
    useGoogleLogin(
      async (credential) => {
        setError("");
        setIsLoading(true);
  ...
  return (
    ...
    <div className="min-h-screen w-full flex items-center justify-center px-4 bg-[radial-gradient(ellipse_at_top_left,_var(--tw-gradient-stops))] from-red-50 via-white to-pink-50 relative overflow-hidden">
      ...
      <button
        onClick={() => navigate("/")}
        className="absolute top-6 left-6 z-10 flex items-center gap-2 px-4 py-2 rounded-full bg-white/60 border border-white shadow-sm backdrop-blur-sm hover:shadow-md transition-all group cursor-pointer"
        aria-label="메인 페이지로 이동"
      >
        <ArrowLeft className="w-4 h-4 text-gray-600 group-hover:text-[#C93831] transition-colors" />
        <span className="text-sm font-medium text-gray-600 group-hover:text-[#C93831]">
          메인으로
        </span>
      </button>
        ...
        <div className="p-8 space-y-6">
          ...
          <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
            ...
            <div className="space-y-1">
              <Label
                htmlFor="employeeId"
                className="text-xs font-bold text-gray-600 ml-1"
              >
                사내 아이디
              </Label>
              ...
              <p className="min-h-[1.25rem] pl-1 text-xs text-red-500">
            ...
            <Button
              type="submit"
              disabled={isLoading}
              className="w-full h-14 rounded-2xl bg-[#C93831] hover:bg-[#B02F28] text-white font-bold text-lg shadow-lg hover:shadow-xl transition-all disabled:opacity-70 active:scale-[0.98] cursor-pointer"
            >
              {isLoading ? "로그인 중..." : "로그인"}
            </Button>
          </form>
          ...
          <div className="relative py-2">
            ...
            <div className="relative flex justify-center text-sm">
              <span className="px-4 bg-white/60 text-gray-500 rounded-full backdrop-blur-sm">
                SNS 간편 로그인
              </span>
            </div>
          </div>
          ...
          <div className="flex justify-center items-center gap-5 pb-2">
            ...
            <SocialButton
              ...
              onMouseEnter={preloadGoogleScript}
              src="/google-logo.png"
              alt="Google"
