    ...
    const handleCallback = async () => {
      ...
      try {
        ...
        if (
          ...
          axiosError.response?.data?.errorCode === "OAUTH_NOT_LINKED"
        ) {
          sessionStorage.removeItem("kakao_oauth_state");
          sessionStorage.removeItem("kakao_oauth_mode");
        ...
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              ...
              <br />
              먼저 사내 아이디로 로그인 후{" "}
              <strong>[마이페이지 &gt; 계정 연동]</strong>을 진행해주세요.
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
            <AlertDialogAction
              onClick={handleConfirmLinkError}
              className="bg-[#C93831] hover:bg-[#B02F28]"
            >
              로그인하러 가기
            </AlertDialogAction>
          </AlertDialogFooter>
